...
from combine.core.sia2d_adapted import Upstream2D as Up2D_new
from oggm.core.sia2d import Upstream2D as Up2D_old
from combine.core.test_cases import Giluwe, Borden
from combine.core.utils import NonRGIGlacierDirectory
from combine.core import gis
from combine.core.massbalance import ClippedLinearMassBalance
from combine.core.visualization import MidpointNormalize
from combine.core.visualization import plot_differences_discrete_cmap
from oggm import cfg
import salem

cfg.initialize()
basedir = '/home/philipp/final/sia2d'
...
old = np.load(os.path.join(basedir, '{:s}_old.npy'.format(case.name)))
new = np.load(os.path.join(basedir, '{:s}_new.npy'.format(case.name)))
diff = new - old
...
plot_differences_discrete_cmap(
    diff, os.path.join(basedir, '{:s}_diff.pdf'.format(case.name)),
    case, cbar_min=cbar_min, cbar_max=cbar_max, norm=norm,
    cbar_label='Surface elevation difference (m)', cmap='PuOr_r', n=16)

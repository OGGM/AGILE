...
from combine.core.arithmetics import para_width_from_thick
from combine.core.arithmetics import para_thick_from_section
...
class Flowline(Centerline):
    ...
    def __init__(self, line=None, dx=1, map_dx=None,
        ...
        if type(bed_h) != torch.Tensor:
            self.bed_h = to_torch_tensor(bed_h, self.torch_type)
        else:
            self.bed_h = bed_h

        # TODO: detach needed because we do not want gradient of thickness
        thick = (surface_h - self.bed_h).detach()
        # thick = surface_h - self.bed_h
        self._thick = torch.clamp(thick,
                                  min=0)
...
class ParabolicBedFlowline(Flowline):
    ...
    def __init__(self, line=None, dx=None, map_dx=None,
        ...
        if type(bed_shape) != torch.Tensor:
            self.bed_shape = to_torch_tensor(bed_shape, self.torch_type)
        else:
    ...
    @property
    def widths_m(self):
        """Compute the widths out of H and shape, use defined function for
        gradient computition
        """
        # return torch.sqrt(4. * self.thick / self.bed_shape) # original
        return para_width_from_thick.apply(self.bed_shape, self.thick)
...
class RectangularBedFlowline(Flowline):
    ...
    def __init__(self, line=None, dx=None, map_dx=None,
        ...
        self._widths = to_torch_tensor(widths, self.torch_type)
...
class TrapezoidalBedFlowline(Flowline):
    ...
    def __init__(self, line=None, dx=None, map_dx=None, surface_h=None,
        ...
        self._lambdas = to_torch_tensor(lambdas, self.torch_type)
...
class FlowlineModel(object):
    ...
    def __init__(self, flowlines, mb_model=None, y0=0., glen_a=None,
        ...
        if fs is None:
            fs = cfg.PARAMS['fs']
        self.glen_a = to_torch_tensor(glen_a, self.fls[0].torch_type)
        self.fs = to_torch_tensor(fs, self.fls[0].torch_type)
...
class FluxBasedModel(FlowlineModel):
    ...
    def step(self, dt):
        ...
        # Shape factor TODO: Implement shape factor function
        sf_stag = 1
        ...
        m_dot = m_dot * torch.where((m_dot > 0.) & (w == 0),
                                    torch.tensor(10.,
                                                 dtype=dtype,
                                                 requires_grad=False),
                                    w)
        ...
        # Next step
        self.t += dt_use
        # print('time ' + str(self.t) + ': ' + str(self.fls[0].thick))
        return dt_use

...
def calculate_default_oggm_statistics(gdir):
    ...
    with xr.open_dataset(
            gdir.get_filepath('model_diagnostics',
                              filesuffix='_oggm_default_past')) as ds:
        diag_past = ds.load()
    ...
    with xr.open_dataset(
            gdir.get_filepath('model_diagnostics',
                              filesuffix='_oggm_default_future')) as ds:
        diag_future = ds.load()
    ...
    for obs_key in obs_given.keys():
        ...
        for year_key in obs_given[obs_key].keys():
            if obs_key_name in ['fl_surface_h', 'fl_widths', 'us']:
                ...
                raise NotImplementedError(f'{obs_key_name}')
    default_oggm_statistics['observations_stats'] = obs_stats
    ...
    default_oggm_statistics['controls_stats'] = controls_stats
    ...
    default_oggm_statistics['past_evol_stats'] = past_evol_stats
    ...
    default_oggm_statistics['today_state_stats'] = today_state_stats
    ...
    default_oggm_statistics['future_evol_stats'] = future_evol_stats

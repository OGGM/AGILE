...
def get_costs(reg_parameters, ref_surf, ref_ice_mask, ref_inner_mask, guessed_bed,
    ...
    if reg_parameters[2] != 0:
        ...
        cost[2] = reg_parameters[2] * (((model_surf - guessed_bed)
    ...
    if reg_parameters[4] != 0:
        ...
        cost[4] = reg_parameters[4] * ((ddit_dx.pow(2).sum() + ddit_dy.pow(2).sum())
                                       / n_inner_mask)
    ...
    if reg_parameters[5] != 0:
        ...
        cost[5] = reg_parameters[5] * ((ddb_dx.pow(2).sum() + ddb_dy.pow(2).sum())
                                       / (2. * n_inner_mask))
    ...
    if reg_parameters[6] != 0:
        ...
        cost[6] = reg_parameters[6] * ((ddb_dx.pow(2).sum() + ddb_dy.pow(2).sum())
                                       / (2 * (ref_ice_mask
    ...
    if reg_parameters[7] != 0:
        ...
        cost[7] = reg_parameters[7] * ((dds_dx.pow(2).sum() + dds_dy.pow(2).sum())
                                       / n_inner_mask)

    if reg_parameters[8] != 0:
        lmsd = LocalMeanSquaredDifference.apply
        cost[8] = reg_parameters[8] * lmsd(model_surf, ref_surf, ref_ice_mask,

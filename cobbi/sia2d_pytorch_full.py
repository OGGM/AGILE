...
from oggm.cfg import RHO, G, N, SEC_IN_YEAR, SEC_IN_DAY, SEC_IN_MONTH
...
class Model2D(object):
    ...
    def __init__(self, bed_topo, init_ice_thick=None, dx=None, dy=None,
        ...
        self.dx = torch.tensor(dx, dtype=torch.float, requires_grad=True)
        self.dy = torch.tensor(dy, dtype=torch.float, requires_grad=True)
        self.dxdy = dx * dy
    ...
    def reset_y0(self, y0):
        ...
        self.y0 = y0
        self.t = torch.tensor(0., dtype=torch.float, requires_grad=True)
...
class Upstream2D(Model2D):
    ...
    def __init__(self, bed_topo, init_ice_thick=None, dx=None,
        ...
        self.gamma = self.glen_a * (RHO * G) ** N / (N + 2) * \
                     torch.tensor(2., dtype=torch.float, requires_grad=True)

        # forward time stepping stability criteria
        # default is just beyond R. Hindmarsh's idea of 1/2(n+1)

        self.cfl = torch.tensor(cfl, dtype=torch.float, requires_grad=True)

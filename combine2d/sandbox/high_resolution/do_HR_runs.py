...
from combine2d.core.test_cases import BordenHR, Trikora
...
if case is BordenHR:
    ...
    reg_parameters_set = np.array([0.2, 1.25, 1e5, 1e8, 0, 0])  # final2
    reg_parameters_set = np.array([0.2, 1.25, 1e4, 1e7, 0, 0])
...
if run_identical_twin:
    reg_parameters_list = [
        np.array([0.2, 1.25, 1e4, 1e7, 0, 0]),
        np.array([0.2, 1.25, 1e4, 1e8, 0, 0]),
        np.array([0.2, 1.25, 1e5, 1e7, 0, 0]),
        np.array([0.2, 1.25, 5e4, 1e7, 0, 0]),
        np.array([0.2, 1.25, 5e4, 1e8, 0, 0]),
        np.array([0.2, 0.75, 1e4, 1e7, 0, 0]),
        np.array([0.2, 0.75, 2e4, 2e7, 0, 0]),
        np.array([0.2, 0.1, 2e4, 2e7, 0, 0]),
        np.array([0.2, 1.25, 2e4, 2e7, 0, 0]),
        ]
    i = -1
    #for i, reg_parameters_set in enumerate(reg_parameters_list[0:1]):
    identical_twin_inversion_settings = get_my_inversion_settings(
        'identical-twin {:d}'.format(i), reg_parameters_set)
...
if run_promised_land:
    ...
    for rmse in [10, 6, 2]:#[2, 4, 6, 8, 10, 12]:

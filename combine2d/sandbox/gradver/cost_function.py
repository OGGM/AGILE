...
"""
Adapted code of cost function for gradient_verification
"""

def create_cost_func(gdir, data_logger=None, use_AD=False,
...
def cost_function(b, reg_parameters, ref_surf, ref_ice_mask, ref_inner_mask,
                  ...
                  data_logger=None, use_AD=False, torch_backward_param=None):
    """
    ...
    use_AD: bool
        whether or not to use PyTorch AD for gradient calculation.
        Determines whether created cost function gives just cost or
        [cost, gradient]
    torch_backward_param: scalar
        Exploit 'special' parameter of torch if not None. Only has effect if
        use_AD = True
    ...
    if use_AD:
        if torch_backward_param is None:
            c.backward()  # This is where the magic happens
        else:
            c.backward(torch.tensor(torch_backward_param, dtype=torch.float))
